...
| --- | --- | --- |
| `format` | `auto` | `json`, `json5`, `yaml`, `toml`, `ini`, `xml`, `hcl`, `properties`, or `auto` to sniff |
...
JSON5, XML, HCL and Java properties behind `@kronajs/element/json5`, `/xml`,
`/hcl` and `/properties`, so a page that never opens one does not carry its
scanner.
