...
**Сворачиваемое дерево и построчное сравнение конфигурационных файлов — React-компонент.**
...
| `Krona.Lines` | просмотр или панель | Текст документа, подсветка и плейсхолдеры свёрнутого |
...
Krona 0.1 показывает один конфигурационный файл или два рядом. Всё, что ниже,
...
| Редактирование | Только чтение оставляет модель неизменяемым списком строк — именно поэтому свёртка и дифф стоят на одной основе. Редактирование не расширило бы этот размен, а поменяло бы его. | Не планируется |
