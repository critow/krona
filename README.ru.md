...
Пока только для просмотрщика. Ссылка внутрь диффа должна называть ещё и версию,
а это вопрос, у которого больше одного разумного ответа.
...
**В npm его пока нет.** Пакет собирается и тестируется здесь, страница демо выше
работает на нём, но для первой публикации нужен trusted publisher, который npm
не даёт завести для несуществующего пакета — см. [RELEASING.md](./RELEASING.md).
Всё описанное ниже — то, как это будет работать; сегодня оно работает из
репозитория.
