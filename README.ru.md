...
`selectedLine` выделяет одну строку: раскрывает то, что её прячет — свёрнутый
блок, — доскроллит до неё и помечает. `onSelectLine` — обратное направление, он
вызывается, когда читатель выбирает строку, и заодно добавляет на строку
действие-ссылку.
...
Оба считают с 1 — так считает жёлоб и так `#L42` означает сорок вторую строку.
Как выглядит ссылка, решаете вы: Krona не знает URL вашей страницы и не выдумывает
его.

**В диффе строка называет ещё и версию**, и какую — говорит `selectedSide`:
по умолчанию `'right'`, текущая, потому что ради неё дифф обычно и открывают.
В `onSelectLine` сторона приходит вместе со строкой.
...
Ссылка называет строку, но раскрывает **строку выравнивания**, на которой та
стоит: обе панели встают на одно и то же сравнение и каждая отмечает свою
сторону. Ради этого читатель по ссылке в дифф и приходит — и поэтому ссылка
несёт строку и сторону, а не номер строки выравнивания. Этот номер —
единственное число, которого нет на экране, и он смещается, стоит поменяться
любой из версий или настройкам сравнения; номер строки стоит в жёлобе и никуда
не денется.
...
Стили каждый элемент приносит с собой, в собственный shadow root, и приносит их
constructed stylesheet: CSS страницы до строк не дотянется, CSS Krona наружу не
протечёт, а Content-Security-Policy нечего запрещать — CSSOM под `style-src` не
подпадает. Сам shadow root **открытый**: скрипты страницы в него по-прежнему
заглядывают, изолированы стили, а не DOM. Темизация не меняется — CSS-переменные
...
Файл, который показывает Krona, — недоверенный ввод, и библиотека устроена так,
чтобы худшее, на что он способен, — выглядеть неправильно:

- **Содержимое попадает на страницу только текстовыми нодами.** Ни `innerHTML`,
  ни автолинковки, ни атрибута, собранного из документа. Первое держит линтер,
  остальное — тесты: `<script>` в значении рисуется символами, а значение
  `javascript:` не становится ссылкой.
- **Из файла никогда не строится JavaScript-объект**, поэтому `__proto__` и
  `constructor` — ключи как любые другие, а prototype pollution нечего
  загрязнять. Реестр — `Map`, пути — строки, состояние строк — типизированные
  массивы.
- **У каждой границы парсера есть значение по умолчанию** в `DEFAULT_LIMITS` —
  размер, число строк, глубина, число диапазонов, длина токенизируемой строки,
  валидация YAML, — и за любой из них документ деградирует до plain text с
  диагностикой, а не во вставшую вкладку. Поиск — буквальный проход, никогда не
  регулярное выражение. YAML-алиасы не разворачиваются.
- **Bidi-контролы и невидимые символы рисуются плашками `U+XXXX`** — и в
  строках, и в подсказке копирования пути, которая плашку нарисовать не может.

Что остаётся на хосте: `labels`, `className`, `style`, `locale`, лимиты разбора
и имена файлов для `unifiedPatch` — доверенные данные, передавайте их из своего
кода, а не из документа. Страница с Content-Security-Policy передаёт свой nonce
в `styleNonce` или подключает `kronajs/styles.css` сама; кастомным элементам не
нужно ни то, ни другое — их таблица стилей constructed, а не встроенная.

Всё, что нарушает любое из этих обещаний, — уязвимость; приватный канал для
сообщения о ней описан в [SECURITY.md](./SECURITY.md).
...
Krona показывает один конфигурационный файл или два — рядом или одной
колонкой, — даёт [править](#редактирование) одиночный и [искать](#поиск) в любом,
[ссылаться на строку](#ссылка-на-строку) — в одном документе и в сравнении — и
выгружать дифф [unified-патчем](#справочник). Всё это в React, а просмотр и сравнение —
[и без фреймворка](#без-react-krona-viewer-и-krona-diff).
...
pnpm test:coverage  # оба набора тестов, один отчёт о покрытии
pnpm bench
pnpm verify         # линт, типы, все тесты с покрытием, сборка
```
...
Покрытие считается сразу по обоим наборам: половину Krona гоняет node, половину —
браузер, и любая из двух цифр по отдельности врёт про другую. Пороги — это пол,
выставленный чуть ниже текущего уровня: они существуют, чтобы поймать фичу,
приехавшую без тестов, а не чтобы их полировали.
...
Пуш тега `v*` публикует три пакета в npm с provenance и открывает GitHub
Release. Шаги — в [RELEASING.md](./RELEASING.md); секретов у репозитория нет
вовсе: npm доверяет workflow через OIDC, а публикующий job не выполняет ни
строчки кода самого репозитория.
