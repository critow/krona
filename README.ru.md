...
[ссылаться на строку](#ссылка-на-строку) и выгружать дифф
[unified-патчем](#справочник). Всё это в React, а просмотр и сравнение —
...
| --- | --- | --- |
| Поиск, редактирование, действия на строке и миникарта в `<krona-viewer>` | Элемент пока умеет читать. Каждый пункт — работа, а не перенос, а тем, у кого есть React, всё это уже доступно. | Да, по мере запросов |
| Одноколоночный дифф в `<krona-diff>` | Ядро это уже считает — `unifiedEntries`, — так что здесь только отрисовка, а не логика. | Да |
| Ссылка на строку внутри диффа | `#L42` в сравнении обязан назвать ещё и версию, и у этого больше одного разумного ответа. Ждём реального применения, чтобы выбрать. | Не решено |
