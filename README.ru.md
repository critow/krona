...
**Поиск, редактирование, действия на строке и миникарта пока только в
`kronajs`.** Если нужно это и React доступен, берите тот
пакет.
...
| --- | --- | --- |
| Поиск, редактирование, действия на строке и миникарта в кастомных элементах | Элемент пока умеет читать. Каждый пункт — работа, а не перенос, а тем, у кого есть React, всё это уже доступно. | Да, по мере запросов |
