...
| `styleNonce` | `string` | — | Nonce для этого `<style>`: для страниц, чей Content-Security-Policy требует его в `style-src` |
...
| `maxLines` | 1 000 000 | Plain text; остаются первый миллион строк, остальное отбрасывается, диагностика-ошибка |
...
который делает git, когда его эвристики сдаются. За `maxInputLength` — по
умолчанию это лимит разбора — точный алгоритм не запускается вовсе и приближение
возвращается сразу: таймаут ограничивает самого Майерса, а разбиение файлов,
хеширование строк для якорей и построение списка рядов происходят до того, как
на часы вообще посмотрят.
