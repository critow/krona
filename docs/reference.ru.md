...
| Проп | Тип | По умолчанию | Описание |
| --- | --- | --- | --- |
| `selectedLine` | `number` | — | Выделить строку: раскрыть то, что её прячет, доскроллить и отметить. Счёт с 1 |
| `selectedSide` | `'left' \| 'right'` | `'right'` | В какой версии считается `selectedLine` |
| `onSelectLine` | `(line: number, side: 'left' \| 'right') => void` | — | Вызывается со строкой и её версией, когда читатель её выбирает. Добавляет на строку действие-ссылку |

Ссылка называет строку, а раскрывает строку выравнивания, на которой та стоит:
обе панели встают на одно сравнение и каждая отмечает свою сторону.
...
Сами дефолты тоже экспортируются — для своего контрола, который должен называть
те же вещи, что и Krona:

| Экспорт | Что делает |
| --- | --- |
| `createDefaultLabels(locale?)` | Полный набор английских строк, числа отформатированы под `locale` |
| `resolveLabels(overrides?, locale?)` | Дефолты со слитыми поверх переопределениями |
...
| `unifiedPatch(rows, leftLines, rightLines, options?)` | Дифф как unified-патч — текст, который печатает `diff -u` и читает `git apply`. Стороны передаются строками — тем, что уже лежит в результате `diffLines`: `result.left` и `result.right` |
